...
import type { ApiResponse, SponsorsData, ContributorsApiData, RecentContributorsApiData } from '~/types';
 ...
 * 获取赞助者数据
 */
export async function getSponsors(controller?: AbortController): Promise<SponsorsData> {
 ...
 * 获取最近贡献者列表
 */
export async function getRecentContributors(limit: number = 5): Promise<string[]> {
  try {
...
export async function incrementCounter(id: number = 1): Promise<void> {
  // 只在生产环境域名下进行统计
  if (typeof window !== 'undefined' && window.location.hostname === 'lightframe.vilinko.com') {
    try {
      await fetch(`${API.BASE_URL}${API.ENDPOINTS.COUNTER_ADD}?id=${id}`).catch(err => 
        console.log('统计请求失败:', err)
      );
    } catch (error) {
